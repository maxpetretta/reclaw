[Reclaw init] Apply memory-guidance updates now.
...
- In `## Every Session`, replace the step that says:
  ...
  with Reclaw behavior (no daily memory file requirement).
...
- If a previous `<!-- BEGIN RECLAW MEMORY GUIDANCE --> ... <!-- END RECLAW MEMORY GUIDANCE -->` block exists, replace that block in place (do not duplicate).
...
- Keep existing manual content unless it conflicts with Reclaw mode.
- If a previous `<!-- BEGIN RECLAW MEMORY NOTICE --> ... <!-- END RECLAW MEMORY NOTICE -->` block exists, replace it in place (do not duplicate).
