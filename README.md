...
Writes daily memory files to `memory/YYYY-MM-DD.md` with structured sections (`Decisions`, `Facts`, `Interests`, `Todo`, `Sessions`). Updates `MEMORY.md` and `USER.md` via a synthesis agent. (OpenClaw mode retains the original section format.)
...
Writes Zettelclaw-compatible journal entries to `03 Journal/YYYY-MM-DD.md` matching the vault template (`Log`, `Todo`, `Sessions`). Updates `MEMORY.md` and `USER.md` in the OpenClaw workspace.
